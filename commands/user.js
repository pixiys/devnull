{
 color:{
  ...
  func: function(bot, msg, command, args){
   ...
   if (args[0] == 'none'){msg.member.removeRoles(colorRoles).then(msg.channel.send({ embed: { color: config.embedColor, description: `Removed ${msg.member.displayName}\'s color.`}}));return;}
   ...
   setTimeout(function() {msg.member.addRole(colorChoose);}, 200);
